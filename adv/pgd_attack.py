...
# from at_implementations.utils import replicate_input
...
def perturb_iterative(
    ...
    assert len(predict) == len(predict_dis)
...
class pgd_attacker:
    def __init__(self, config):
        ...
        self.predict_dis = config['predict_dis']
        self.attack = LinfPGDAttack(predict=self.predict, predict_dis=self.predict_dis, nb_iter=5)

        self.clean_train_dataloader = config['clean_train_dataloader']
        self.perturbed_train_dataloader = None

    def perturb_train_dataloader(self):
        # print()
        self.perturbed_train_dataloader = []
        device = self.config['device']
        for i, (x, y) in enumerate(self.clean_train_dataloader):
            print(i)
            x = x.to(device)
            y = y.to(device)

            perturbed_x = self.attack.perturb(x, y)

            self.perturbed_train_dataloader.append((perturbed_x, y))
