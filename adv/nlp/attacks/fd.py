...
from OpenAttack.attack_assist.substitute.word import (
    WordSubstitute,
...
from OpenAttack.utils import check_language, get_language, language_by_name
...
class FDAttacker(ClassificationAttacker):
    ...
    def attack(self, victim: Classifier, x_orig, goal: ClassifierGoal):
        ...
        for i in range(self.max_iter):
            ...
            prob = prob[0]
            if (
                grad.shape[0] != len(sent)
                or grad.shape[1] != victim_embedding.embedding.shape[1]
            ):
                raise RuntimeError(
                    "Sent %d != Gradient %d" % (len(sent), grad.shape[0])
                )
            s1 = np.sign(grad[idx])
            ...
            for word in reps:
                s0 = np.sign(
                    victim_embedding.transform(word, self.token_unk)
                    - victim_embedding.transform(sent[idx], self.token_unk)
                )
                v = np.abs(s0 - s1).sum()
                if goal.targeted:
                    v = -v
