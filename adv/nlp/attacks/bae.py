...
class BAEAttacker(ClassificationAttacker):
    ...
    def __init__(
        ...
        if self.replace_rate == 1.0 and self.insert_rate == 0.0:
            ...
            self.sub_mode = (
                3  # first replacement AND then insertion for each token / subword
            )
        else:
    ...
    def attack(self, victim: Classifier, sentence, goal: ClassifierGoal):
        ...
        word_predictions = self.mlm_model(input_ids_.to(self.device))[
            0
        ].squeeze()  # seq-len(sub) vocab
        word_pred_scores_all, word_predictions = torch.topk(
            word_predictions, self.k, -1
        )  # seq-len k
        ...
        for top_index in list_of_index:
            ...
            # in the substitute function, masked_index = top_index[0] + 1, because "[CLS]" has been inserted into
            # sub_words
            ...
            if self.sub_mode == 0:
                ...
                substitutes_replace = self.get_substitues(
                    top_index[0] + 1,
                    sub_words,
                    tokenizer,
                    self.mlm_model,
                ...
                substitutes_insert = self.get_substitues(
                    top_index[0] + 1,
                    sub_words,
                    tokenizer,
                    self.mlm_model,
                ...
                replace_sub_len, insert_sub_len = len(substitutes_replace), len(
                    substitutes_insert
                )
            ...
            for i, substitute in enumerate(substitutes):
                ...
                # From TextAttack's implementation: Finally, since the BAE code is based on the TextFooler code,
                # we need to adjust the threshold to account for the missing / pi in the cosine similarity
                ...
                input_ids = (
                    torch.tensor(inputs["input_ids"]).unsqueeze(0).to(self.device)
                )
    ...
    def get_substitues(
        self, masked_index, tokens, tokenizer, model, sub_mode, k, threshold=3.0
    ):
