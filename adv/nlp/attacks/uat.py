...
class UATAttacker(ClassificationAttacker):
    ...
    def attack(self, victim: Classifier, sentence: str, goal: ClassifierGoal):
        trigger_sent = self.tokenizer.detokenize(
            self.triggers + self.tokenizer.tokenize(sentence, pos_tagging=False)
        )
    ...
    def get_triggers(
        ...
        for tag in requires:
            if tag not in victim.TAGS:
                raise AttributeError(
                    "`%s` requires victim to support `%s`"
                    % (self.__class__.__name__, tag.name)
                )
        ...
        for epoch_idx in range(epoch):
            for num_iter in tqdm(
                ...
                x = [tokenizer.tokenize(sent, pos_tagging=False) for sent in batch["x"]]
                y = batch["y"]
