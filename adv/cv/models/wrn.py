...
def test_under_carlini_attack(model, device, test_loader, attacks, level=0):
    ...
    predictions = torch.zeros(
        len(threshold), len(attacks), len(test_loader), 2, test_loader.batch_size
    ).cuda()

    mixture_output = torch.zeros(
        len(threshold), len(attacks), len(test_loader), test_loader.batch_size
    ).cuda()
        ...
        for id_attack, attack in enumerate(attacks):
            ...
            for k in range(len(threshold)):
                ...
                with torch.no_grad():
                    ...
                    mixture_output[k][id_attack][batch_idx] = (
                        predictions[k][id_attack][batch_idx][0].eq(targets).int()
                        * model.weights[0]
                        + predictions[k][id_attack][batch_idx][1].eq(targets).int()
                        * model.weights[1]
                    )

        accuracy = (
            mixture_output[:, :, : batch_idx + 1, :].min(1)[0].sum(dim=1).sum(dim=1)
            / total
        )
        for att in range(len(attacks)):
            print(
                "From the " + str(att) + "th attack, for each threshold : ",
                (
                    mixture_output[:, :, : batch_idx + 1, :]
                    .min(1)[1]
                    .eq(att)
                    .sum(dim=1)
                    .sum(dim=1)
    ...
    return [(threshold[t], 100.0 * accuracy[t].item()) for t in range(len(threshold))]
...
def test_under_attack(model, device, test_loader, attack, adversary="pgd", level=0):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        ...
        if adversary == "carlini":
            ...
            for k in range(len(threshold)):
                ...
                print(
                    level * "   " + "Number of adversarial example rejected  = ", compt
                )
                ...
                with torch.no_grad():
                    ...
                    if hasattr(model, "classifiers"):
                        for i in range(len(model.classifiers)):
                            ...
                            _, predicted = outputs.max(1)
                            corrects[k] += (
                                predicted.eq(targets).sum().item() * model.weights[i]
                            )
                    else:
                    ...
                    print(
                        level * "   "
                        + "Threshold : %.3f%% - Acc: %.3f%% (%.3f/%d)"
                        % (
                            threshold[k],
                            100.0 * corrects[k] / totals[k],
                            corrects[k],
                            totals[k],
                        )
                    )
    ...
    if adversary == "carlini":
        ...
        return [
            (threshold[t], 100.0 * corrects[t] / totals[t])
...
def save_model(
    save_dir, i, device, model, accuracy, accuracy_under_attack, epoch, level=0
):
    print(level * "   " + "Saving..")
    ...
    if epoch != -1:
        ...
        torch.save(
            checkpoint, save_dir + "/model_" + str(i) + "/epoch_" + str(epoch) + ".pth"
        )
    else:
...
# Method to load a classifier at a given training epoch or the best one (according to the natural accuracy or the
# accuracy under attack)
def load_model(save_dir, i, epoch, device, number_of_class, top_acc_under_attack=True):
    if epoch != -1:
        checkpoint = torch.load(
            save_dir + "/model_" + str(i) + "/epoch_" + str(epoch) + ".pth"
        )
    elif not top_acc_under_attack:
        checkpoint = torch.load(save_dir + "/topAccuracy/model_" + str(i) + ".pth")
    else:
        checkpoint = torch.load(
            save_dir + "/topAccuracyUnderAttack/model_" + str(i) + ".pth"
        )
...
def updateAndSaveBestAccuracies(
    save_dir, i, device, model, accuracy, accuracy_under_attack, level=0
):
    acc, acc_under_attack = model.module.updateBestAccuracies(
        accuracy, accuracy_under_attack
    )
    ...
    if acc_under_attack:
        save_model(
            save_dir + "/topAccuracyUnderAttack",
            i,
            device,
            model,
            accuracy,
