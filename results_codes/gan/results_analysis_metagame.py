...
from results_codes.gan import results_analysis_output
...
for gan_name in gan_name_list:
    for dataset in dataset_list:
        ...
        for solution in solution_list:
            ...
            # )
            # print(res)
            sim_matrix_full = getattr(results_analysis_output, '{}_{}_{}'.format(gan_name, solution, dataset))
            # print(res)
            for i in [0, 1]:
                sim_matrix = sim_matrix_full[i]
                # print(sim_matrix.shape)
                heat_map_lin = sns.heatmap(sim_matrix,
                                           annot=True, cmap="magma", linewidths=.25)
                heat_map_lin.invert_yaxis()
                plt.savefig(osp.join(figure_dir, "{}_{}_{}_{}.pdf".format(dataset, gan_name, solution,
                                                                          'gen' if i == 0 else 'dis')),
