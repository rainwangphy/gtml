...
class ActorCritic(nn.Module):
    ...
    def act(self, state):
        ...
        if self.has_continuous_action_space:
            ...
            dist = Normal(loc=action_mean, scale=scale)
            #########################################
            # dist = MultivariateNormal(action_mean, cov_mat)  ## 3/4  # 替换这个
            action = dist.sample()
            action_logprob = dist.log_prob(action)
            action_logprob = action_logprob.sum(dim=-1, keepdim=False)

        else:
            action_probs = self.actor(state)
            dist = Categorical(action_probs)
            action = dist.sample()
            action_logprob = dist.log_prob(action)  ## 0.6-1
    ...
    def evaluate(self, state, action):
        ...
        if self.has_continuous_action_space:
            ...
            action_logprobs = dist.log_prob(action).sum(
                dim=-1, keepdim=False
            )  # should be torch.Size([500])
            # action_logprobs = dist.log_prob(action)
            dist_entropy = dist.entropy().sum(
                dim=-1, keepdim=False
            )  # should be torch.Size([500])
