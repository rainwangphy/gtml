...
class ImageDataset(Dataset):
    def __init__(self, path, exts=["png", "jpg"]):
        self.paths = []
        for ext in exts:
            self.paths.extend(list(glob(os.path.join(path, "*.%s" % ext))))
...
def get_fid(
    ...
    **kwargs,
    ...
    Returns:
        FID: float
    """
    (acts,) = get_inception_feature(images, dims=[2048], use_torch=use_torch, **kwargs)
...
def get_inception_score(
    ...
    **kwargs,
    ...
    """
    (probs,) = get_inception_feature(images, dims=[1008], use_torch=use_torch, **kwargs)
    inception_score, std = calculate_inception_score(probs, splits, use_torch)
