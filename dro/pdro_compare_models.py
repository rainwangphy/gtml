...
def select_model(
    ...
    for save_name in save_names:
        ...
        if best_only:
            ...
            _, best_adv = compare_models(
                losses_model.reshape(1, -1), log_weights_advs, baseline=0
            )
...
def get_args():
    ...
    args.add_argument("--no-cuda", action="store_true", help="Force CPU")
    args.add_argument("--save-names", type=str, nargs="+")
    args.add_argument("--train-log-interval", type=int, default=1)
    args.add_argument(
        "--select-by", type=str, default="adv", choices=["adv", "erm", "robust"]
    )
    args.add_argument(
        "--filter-advs-by",
        type=str,
    ...
    args.add_argument(
        "--use-accs", action="store_true", help="Use accuracies rather than loss"
    )
...
def main():
    ...
    if args.n_reruns is None:
        ...
        print(best_save)
        average, robust = test_metrics(best_save, select_by=args.select_by)
        ...
        for run_id in range(args.n_reruns):
            ...
            print(best_save)
            run_average, run_robust = test_metrics(best_save, select_by=args.select_by)
