...
def register_architecture(alt_name=None):
    def register_architecture_fn(fn):
        ...
        if name in architecture_list:
            raise ValueError(f"Cannot register duplicate architecture ({name})")
        elif alt_name is not None and alt_name in architecture_list:
            raise ValueError(f"Cannot register duplicate architecture ({alt_name})")
...
def bert_base_uncased(input_shape, output_size):
    return BERT.from_pretrained("bert-base-uncased", cache_dir=_PRETRAINED_MODELS_PATH)
...
def gpt2(input_shape, output_size):
    return GPT2.from_pretrained("gpt2", cache_dir=_PRETRAINED_MODELS_PATH)


@register_architecture()
def gpt2_untrained(input_shape, output_size):
    model = GPT2.from_pretrained("gpt2", cache_dir=_PRETRAINED_MODELS_PATH)
    model.init_weights()
    return model
...
def build_model(name, input_shape, output_size=None, verbose=True):
