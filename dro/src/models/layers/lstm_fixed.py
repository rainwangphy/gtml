...
class MyLSTMCell(nn.LSTMCell):
    def forward(self, x, hx=None):
        ...
        h, c = hx
        # Get gates
        gates = th.einsum("bd,hd->bh", [h, self.weight_hh]) + th.einsum(
            "bd,hd->bh", [x, self.weight_ih]
        )
...
class MyLSTM(nn.Module):
    def __init__(
        ...
        input_dims = [input_size] + [hidden_size * self.num_directions] * (
            num_layers - 1
        )
    ...
    def transduce(self, inputs, h0_fwd, h0_bwd, lengths=None, forward=True):
        ...
        for layer in range(self.num_layers):
            ...
            hl_fwd, _ = self.transduce_layer(layer, hl, hl0, cl0, lengths)
        for i in range(L):
            hs_i, cs_i = [], []
            # Reset the state
            if lengths is not None and not forward and i <= L - min_length:
