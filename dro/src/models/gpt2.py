...
class GPT2ClassConditional(GPT2PreTrainedModel):
    ...
    def __init__(self, config, n_classes, generative=False):
        ...
        self.class_idxs = th.LongTensor(
            [config.vocab_size + idx for idx in range(n_classes)]
        )
        self.generative = generative
        # Create transformer model
        self.transformer = GPT2Model(config)
        # Add class embeddings
        self.transformer.resize_token_embeddings(config.vocab_size + self.n_classes)
        # Output layer
    ...
    def forward(
        ...
        input_ids_with_label = th.cat(
            [
                ...
                input_ids[:, 1:-1],
            ],  # Rest of the sentence(s) up until
