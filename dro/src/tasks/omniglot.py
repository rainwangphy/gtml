...
class Omniglot(Task):
    ...
    def _load_data(self):
        ...
        train_transform = transforms.Compose(
            [
                transforms.RandomApply(
                    [transforms.RandomAffine(10, [0.1, 0.1], fillcolor=255)], 19 / 20
                ),
                transforms.ToTensor(),
        ...
        test_transform = transforms.Compose(
            [transforms.ToTensor(), transforms.Normalize((0.91,), (0.23,))]
        )
        dataset_class = (
            InMemoryOmniglotAlphabet if self._in_memory else OmniglotAlphabet
        )  # noqa
