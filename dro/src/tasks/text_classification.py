...
class MultiNLI(GlueTask):
    ...
    _matched_genres = ["fiction", "government", "slate", "telephone", "travel"]
    _mismatched_genres = ["facetoface", "letters", "nineeleven", "oup", "verbatim"]
...
class BiasedSST(TextClassificationTask):
    ...
    def __init__(
        ...
        super(BiasedSST, self).__init__(
            os.path.join(path, "glue_data", f"SST-2-biased-{self.bias_percent}"),
            "SST-2-biased",
...
class MultiNLISagawa(MultiNLI):
    ...
    def load_data(self):
        ...
        train_idxs = [idx for idx, split_idx in enumerate(split_idxs) if split_idx == 0]
        dev_idxs = [idx for idx, split_idx in enumerate(split_idxs) if split_idx == 1]
        test_idxs = [idx for idx, split_idx in enumerate(split_idxs) if split_idx == 2]
        # Subsample
