...
class SplitMiniImageNet(Task):
    ...
    def _load_data(self):
        ...
        print(len(self._train_data), len(self._valid_data), len(self._test_data))
        # In memory stuff
        if self._in_memory_test and not self._in_memory and self.classes is None:
            valid_cache_file = os.path.join(
                self.path, f"miniimagenet_{self.img_size}_valid.npz"
            )
            self._valid_data = InMemoryCachedDataset(self._valid_data, valid_cache_file)
            test_cache_file = os.path.join(
                self.path, f"miniimagenet_{self.img_size}_test.npz"
            )
            self._test_data = InMemoryCachedDataset(self._test_data, test_cache_file)
