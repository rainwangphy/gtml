...
def _make_lm_fn(fn):
    ...
    def lm_fn(*args, **kwargs):
        ...
        lm_task = LanguageModelingTask.from_text_task(task)
...
def MiniImageNet256(path="./datasets", model_name=None):
    """100 classes of MiniImageNet split into 20 5-way classification tasks"""
    task = SplitMiniImageNet(path, in_memory=False, split=[6, 2, 2], img_size=256)
...
def SST_imbalanced(path="./datasets", model_name=None):
    ...
    pos_idxs = [idx for idx, z in enumerate(task.train_data) if z.label.item() == 1]
    neg_idxs = [idx for idx, z in enumerate(task.train_data) if z.label.item() == 0]
