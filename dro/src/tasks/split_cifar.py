    ...
    def _load_data(self):
        ...
        if self.cached_data and _CACHED_CIFAR100_TRAIN is not None:
            ...
            train_data = datasets.CIFAR100(
                self.path, train=True, download=True, transform=dataset_transform
            )
        ...
        with th.random.fork_rng():
            ...
            self._train_data, self._valid_data = random_split(
                train_data, [len(train_data) - self.valid_split, self.valid_split]
            )
        ...
        if self.cached_data and _CACHED_CIFAR100_TEST is not None:
            ...
            test_data = datasets.CIFAR100(
                self.path, train=False, download=True, transform=dataset_transform
            )
