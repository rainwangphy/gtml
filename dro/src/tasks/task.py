...
class Task(nn.Module):
    ...
    def shatter_batch(self, batch):
        ...
        return [tuple([elem[i] for elem in batch]) for i in range(batch.size)]
...
def concatenate_tasks(
    ...
    if concat_train:
        new_task._train_data = ConcatDataset([task.train_data for task in tasks])
    if concat_valid:
        new_task._valid_data = ConcatDataset([task.valid_data for task in tasks])
    if concat_test:
