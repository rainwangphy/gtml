...
def permuted_MNIST_20(path="./datasets", model_name=None):
    ...
    task_list = [pMNIST(path, valid_split=0, number=i, rng=rng) for i in range(23)]
    input_shape = (1, 28, 28)
    output_size = 10
...
def split_MNIST(path="./datasets", model_name=None):
    ...
    task_list = [
        SplitMNIST(path, digits=set(digits_order[i : i + 2]), valid_split=0)
...
def consistent_split_CIFAR(path="./datasets", model_name=None):
    ...
    task_list = [
        SplitCIFAR100(
            "./datasets",
            classes=set(
                [fine_label_idx[label] for label in coarse_to_fine_labels[coarse_label]]
            ),
            valid_split=0,
            cached_train=True,
        )
...
def omniglot(path="./datasets", model_name=None):
    ...
    task_list = [Omniglot(path, alphabet=a, in_memory=True) for a in alphabet_order]
    input_shape = (1, 28, 28)
    output_size = None
    return task_list, input_shape, output_size
...
def pan_sentiment(path="./datasets", model_name="bert-base-uncased"):
    """All sentiment tasks"""
    task_list = [
        TCDTask(path, task_name="amazon_review_polarity", model_name=model_name),
...
def text_classification(path="./datasets", model_name="bert-base-uncased"):
    ...
    tokenizer = tokenizers.PretrainedTokenizer(model_name)
    task_list = [
        TCDTask(path, task_name="ag_news", model_name=model_name, tokenizer=tokenizer),
        TCDTask(
            path,
            task_name="amazon_review_full",
            model_name=model_name,
            tokenizer=tokenizer,
        ),
        TCDTask(path, task_name="dbpedia", model_name=model_name, tokenizer=tokenizer),
        TCDTask(
            path, task_name="yahoo_answers", model_name=model_name, tokenizer=tokenizer
        ),
        TCDTask(
            path,
            task_name="yelp_review_full",
            model_name=model_name,
            tokenizer=tokenizer,
        ),
        GlueTask(
