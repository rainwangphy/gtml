...
def _check_inputs_jvp(f, x, v):
    ...
    if len(v) != len(x):
        raise ValueError(
            f"params and v must have the same length. " f"({len(v)}=/={len(x)})"
        )
...
def _check_inputs_vjp(f, x, v):
    ...
    if len(f) != len(v):
        raise ValueError(
            f"params and v must have the same length. " f"({len(v)}=/={len(f)})"
        )
...
def vjp(f, x, v, retain_graph=False):
    ...
    # Get df/dx . v
    v_dot_df_dx = th.autograd.grad(
        f, x, grad_outputs=v, create_graph=True, retain_graph=retain_graph
    )
    # Detach
...
def jvp(f, x, v, retain_graph=False):
    ...
    # Get df/dx . z
    z_dot_df_dx = th.autograd.grad(
        f, x, grad_outputs=z, create_graph=True, retain_graph=retain_graph
    )
    # Differentiate wrt. z
...
def hvp(f, x, v, retain_graph=False):
    ...
    if any(f_i.numel() > 1 for f_i in f):
        raise ValueError(f"f must be scalar for HVP (got {[f_i.size() for f_i in f]})")
    # First backprop get df/dx
    df_dx = th.autograd.grad(f, x, create_graph=True, retain_graph=retain_graph)
    # Second backprop
    # (H*v)_i = d/dx_i \sum_j df/dx_j v_j
    return th.autograd.grad(df_dx, x, grad_outputs=v, retain_graph=retain_graph)
