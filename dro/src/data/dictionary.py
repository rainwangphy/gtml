...
class Dictionary(object):
    ...
    def symbol(self, idx):
        if idx > len(self.symbols):
            raise ValueError(
                f"Invalid index {idx} for dictionary of " f"length {len(self.symbols)}"
            )
        else:
    ...
    def string(self, idxs, with_pad=False, with_eos=False, join_with=None):
        ...
        idxs = [
            idx
            for idx in idxs
            if (idx != self.pad_idx or with_pad) and (idx != self.eos_idx or with_eos)
        ]
        # Back to words
    ...
    def from_data(
        ...
        # Filter by frequency
        most_freq = {
            symbol: count for symbol, count in counts.items() if count >= min_count
        }
        ...
        dic = Dictionary(
            symbols=symbols, special_symbols=special_symbols, no_specials=no_specials
        )
