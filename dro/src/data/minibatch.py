...
class BaseMiniBatch(object):
    ...
    def __getitem__(self, idx):
        if isinstance(idx, int):
            ...
            return [self._getitem_single_idx(i) for i in range(*idx.indices(len(self)))]
...
class TupleMiniBatch(BaseMiniBatch):
    ...
    def slice(self, idxs):
        """Select indices from batch"""
        sub_elements = [
            elem[idxs] if hasattr(elem, "__getitem__") else elem for elem in self
        ]
        sub_attributes = {
            k: v[idxs] if hasattr(v, "__getitem__") else v
    ...
    def to(self, device):
        ...
        return TupleMiniBatch(
            [elem.to(device) if isinstance(elem, th.Tensor) else elem for elem in self],
            attributes={
...
def test():
    ...
    batch = TupleMiniBatch(
        (one, two, three, four, 5), attributes=(one, two, three, four, 5)
    )
    # Access all elements
    for idx in range(4):
        print(batch[idx])
