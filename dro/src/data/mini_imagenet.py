...
class MiniImageNet(Dataset):
    ...
    def __init__(self, path, only_labels=None, img_size=84, transform=None):
        ...
        if only_labels is None:
            ...
            self.label_map = {label: idx for idx, label in enumerate(only_labels)}
...
def InMemoryMiniImagenet(
    ...
    if img_size == 256:
        print(
            "Warning: loading the full MiniImageNet in memory. "
    ...
    if cache_path is not None:
        cache_file = os.path.join(cache_path, f"MiniImageNet_{img_size}_full.npz")
    ...
    if only_labels is not None:
        ...
        _cached_data._data = [
            img
            for in_subset, img in zip(subset_idxs, _cached_data._data)
            if in_subset == 1
        ]
