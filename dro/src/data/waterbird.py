...
class WaterbirdDataset(ImageDataset):
    ...
    def from_folder(
        ...
        if cache_prefix is not None and in_memory:
            cache_suffix = "_".join(
                [split, "x".join(str(res) for res in resolution), f"{scale:.2f}"]
            )
        ...
        for metadata in tqdm.tqdm(
            _WATERBIRD_METADATA[split], desc=f"Reading Waterbird {split} data"
        ):
            img_path = os.path.join(path, metadata["img_path"])
            label = th.tensor(metadata["label"])
            f = ImageFeatures(img_path, label, in_memory)
            features.append(f)
            attributes.append({"place": metadata["place"], "idx": metadata["idx"]})
