...
def _load_celeba_metadata(path):
    ...
    with open(attr_path, "r") as fd:
        ...
        for line in fd:
            ...
            attributes[img_name] = {
                name: int(val) for name, val in zip(attr_names, img_attr)
            }
    ...
    with open(split_path, "r") as fd:
        ...
        for line in fd:
            ...
            _CELEBA_METADATA[splits[int(split)]].append(
                {"img_path": img_name, "img_attr": attributes[img_name]}
            )
...
class CelebADataset(ImageDataset):
    ...
    def from_folder(
        ...
        for metadata in tqdm.tqdm(
            _CELEBA_METADATA[split], desc=f"Reading CelebA {split} data"
        ):
            img_path = os.path.join(path, "img_align_celeba", metadata["img_path"])
