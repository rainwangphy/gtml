...
class Tokenizer(object):
    ...
    def encode_plus(
        ...
        output["input_ids"] = [self.token_to_idx[token] for token in self.tok(string)]
        output["token_type_ids"] = [0] * len(string)
        if text_pair is not None:
            output["input_ids"].extend(
                [self.token_to_idx[token] for token in self.tok(text_pair)]
            )
...
class PretrainedTokenizer(Tokenizer):
    ...
    def decode(
        self, idxs, skip_special_tokens=False, clean_up_tokenization_spaces=True
    ):
        """Convert indices to detokenized text"""
        return self._tokenizer.decode(
            idxs, skip_special_tokens, clean_up_tokenization_spaces
        )
