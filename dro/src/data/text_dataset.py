...
class ClassificationExample(InputExample):
    ...
    def to_features(
        ...
        return InputFeaturesWithAttributes(
            input_ids=input_ids, token_type_ids=token_type_ids, label=label
        )
...
class TextDataset(Dataset):
    ...
    def __init__(
        ...
        self.features = [
            InputFeaturesWithAttributes(
                ...
                label=as_tensor(
                    f.label, th.float if isinstance(f.label, float) else th.long
                ),
            )
            for f in features
        ]
    ...
    def from_examples(
        ...
        if cached_filename is not None and os.path.isfile(cached_filename):
            if verbose:
                print(
                    f"Loading features from cached file {cached_filename}", flush=True
                )
        ...
        for (ex_idx, example) in enumerate(examples):
            if verbose:
                if ex_idx % 10000 == 0:
                    print(f"Writing example {ex_idx} of {len(examples)}", flush=True)
    ...
    def filter(self, filter_fn):
        ...
        """
        idxs = [idx for idx in range(len(self)) if filter_fn(self.attributes[idx])]
    ...
    def collate_fn(self, features):
        ...
        return TupleMiniBatch(
            ...
            attributes={
                k: [f.attributes[k] for f in features] for k in features[0].attributes
            },
        )
    ...
    def shatter_batch(self, batch):
        ...
        if self.pad_on_left:
            input_ids = [input_ids[idx, -lengths[idx] :] for idx in range(batch.size)]
            token_type_ids = [
                segment_ids[idx, -lengths[idx] :] for idx in range(batch.size)
            ]
        else:
            input_ids = [input_ids[idx, : lengths[idx]] for idx in range(batch.size)]
            token_type_ids = [
...
def get_test_text_dataset(N, tokenizer):
    ...
    examples = [
        ClassificationExample(
            ...
            text_a=" ".join(
                [
                    vocab[(idx * 11 * w_idx + 5) % len(vocab)]
    ...
    dataset = TextDataset.from_examples(
        ...
        pad_token=tokenizer._tokenizer.convert_tokens_to_ids(
            [tokenizer._tokenizer.pad_token]
        )[0],
        max_length=None,
