...
def train(
    ...
    robust_model_file = os.path.join(results_prefix, f"{save_name}_robust_model.pt")
    ...
    if train_domain_filters is not None:
        ...
        train_domains_data = {
            domain: (
                ...
                if callable(domain_filter)
                else task.train_data.subset(domain_filter)
            )
    ...
    if valid_pseudo_domain_filters is not None:
        valid_pseudo_domains_idxs = {
            domain: (
                [
                    idx
                    for idx, x in enumerate(task.valid_data.attributes)
                    if domain_filter(x)
                ]
                if callable(domain_filter)
                else domain_filter
    ...
    if valid_pseudo_domain_filters is not None:
        adv_stop = GroupRobustStopping(valid_pseudo_domains_idxs, lower_is_better=False)
    ...
    for epoch in range(1, optim_args.n_epochs + 1):
        ...
        for step_in_epoch, batch in enumerate(itr, 1):
            ...
            if adv_args.pdro:
                ...
                if adv_args.ratio_model:
                    ...
                    if adv_args.renorm_ratios:
                        log_q = th.log_softmax(log_q, dim=0) + np.log(len(log_q))
                else:
                    # Get NLL for words
            ...
            if optim_args.l2_reg > 0:
                ...
                model_loss = model_loss + optim_args.l2_reg * th.sum(param_vec ** 2)
            # Model backward pass
            model_loss.backward()
            # Take a step
...
def get_args():
    ...
    general_args.add_argument(
        "--force-save-name",
        type=str,
        default="biased_sst_p_dro",
        help="Force using a specific save name",
    )  # default None
    general_args.add_argument("--no-cuda", action="store_true", help="Force CPU")
    general_args.add_argument(
        "--dry-run",
        action="store_true",
    ...
    general_args.add_argument(
        "--exp-prefix", type=str, default="", include_in_name=True
    )
    general_args.add_argument("--data-dir", type=str, default="datasets")
    general_args.add_argument("--test-only", action="store_true", help="Just test")
    general_args.add_argument(
        "--test-on-split",
        type=str,
        default="test",
    ...
    general_args.add_argument(
        ...
        default=[
            "biased=True,label=0",
            "biased=True,label=1",
            "biased=False,label=0",
            "biased=False,label=1",
        ],
        nargs="*",
    )
    general_args.add_argument(
        "--task",
        default="biased_SST_95",
...
def main():
    ...
    task, input_shape, output_size = prepare_task(
        general_args.task, path=general_args.data_dir, model_name=input_format
    )
    ...
    if not hasattr(task.train_data, "attributes") or task.train_data.attributes is None:
    ...
    if general_args.eval_on_domains is not None:
        ...
        eval_domain_filters = {
            domain: parse_domain(domain) for domain in general_args.eval_on_domains
        }
        print(eval_domain_filters)
    ...
    if dro_args.group_dro:
        if dro_args.group_specifications is not None:
            ...
            train_domain_filters = {
                domain: parse_domain(domain) for domain in dro_args.group_specifications
            }
            ...
            train_domain_filters = {
                str(group): train_idxs[train_group_labels == group] for group in groups
            }
            valid_idxs = np.arange(len(task.valid_data))
            valid_pseudo_domain_filters = {
                str(group): valid_idxs[valid_group_labels == group] for group in groups
            }

        elif dro_args.gold_groups:
            # This is when we are using the same domains as the evaluation
            # domains
            train_domain_filters = {k: v for k, v in eval_domain_filters.items()}
    ...
    for run_id in range(general_args.n_reruns):
        ...
        for stopping_strategy in ["erm", "adv", "robust"]:
            ...
            results_file = (
                f"{run_save_name}_{test_split}_results_{stopping_strategy}.npz"  # noqa
            )
            np.savez_compressed(
